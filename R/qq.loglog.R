...
create.corr <- function(et,corr.tab) {
  ...
  for (i in seq_along(corr.tab$e1)) {
    ...
    pair.vals <- et[,..swap.cols]
    if (corr.tab$log2.fold[i] > 0) {
      swap.targets <- pair.vals[[1]] != pair.vals[[2]]
      swaps.needed <- swaps4odds(table(pair.vals),2^corr.tab$log2.fold[i])
      if (is.infinite(swaps.needed)) {
        print('swaps saturated!')
        p.swap <- 1.0;
      }
      else {
        p.swap <- min(1.0, swaps.needed / sum(swap.targets))
        print(paste(corr.tab$e1[i],corr.tab$e2[i],p.swap))
      }
#!    p.swap <- 1 - 2^-corr.tab$log2.fold[i]
      swap.rows <- swap.targets & (runif(nrow(pair.vals))<p.swap)
      et[swap.rows,(swap.cols) := isFALSE(swap.cols)]
    }
    if (corr.tab$log2.fold[i] < 0) {
      swap.targets <- pair.vals[[1]] == pair.vals[[2]]
      swaps.needed <- -swaps4odds(table(pair.vals),2^corr.tab$log2.fold[i])
...
corr.tab <- data.table(e1=c('event_1','event_2','event_3','event_7'),
                       e2=c('event_4','event_5','event_6','event_17'),
                       log2.fold=c(1,2,-1,-2))
